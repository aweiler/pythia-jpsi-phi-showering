...
namespace Pythia8 {
	...
	Pythia::~Pythia() {
		...
		if (useNewPdfHard && pdfHardAPtr != pdfAPtr) {
			delete pdfHardAPtr;
		}
		if (useNewPdfHard && pdfHardBPtr != pdfBPtr) {
			delete pdfHardBPtr;
		}
		if (useNewPdfA) {
			delete pdfAPtr;
	...
	bool Pythia::setPDFPtr( PDF* pdfAPtrIn, PDF* pdfBPtrIn, PDF* pdfHardAPtrIn,
		...
		if (pdfAPtrIn == 0 && pdfBPtrIn == 0) {
			return true;
		}

		// The two PDF objects cannot be one and the same.
		if (pdfAPtrIn == pdfBPtrIn) {
			return false;
		}
	...
	bool Pythia::init() {
		...
		if (frameType < 4 ) {
			...
			if (skipInit) {
				...
				info.addCounter(2);
				if (nSkipAtInit > 0) {
					lhaUpPtr->skipEvent(nSkipAtInit);
				}
				return true;
			}
			...
			if (frameType == 4 || frameType == 5) {
				...
				if (doMerging && !inputMergingHooks) {
					if (hasOwnMergingHooks && mergingHooksPtr) {
						delete mergingHooksPtr;
					}
			...
			if (abs(idA) == idRenameBeams) {
				idA = 16;
			}
			if (abs(idB) == idRenameBeams) {
				idB = -16;
			}
			if (idA == 0 || idB == 0) {
		...
		if (useSLHAcouplings) {
			couplingsPtr = slhaInterface.couplingsPtr;
		}
		...
		if (show1PD > 0) {
			particleData.list(show1PD);
		}
		if (showCPD) {
			particleData.listChanged(showCRD);
		}
		if (showAPD) {
	...
	bool Pythia::checkBeams() {
		...
		if (isLeptonA && isLeptonB && isUnresolvedA == isUnresolvedB) {
			return true;
		}
	...
	bool Pythia::next() {
		...
		//cout << "ANDI Pythia::next() next event starts " << endl;
		...
		if (doMomentumSpread || doVertexSpread) {
			beamShapePtr->pick();
		}
		...
		for ( ; ; ) {
			...
			if (!doPartonLevel) {
				...
				info.addCounter(4);
				if (doLHA && nPrevious < nShowLHA) {
					lhaUpPtr->listEvent();
				}
				if (nPrevious < nShowInfo) {
					info.list();
				}
				if (nPrevious < nShowProc) {
					process.list(showSaV, showMaD);
				}
				return true;
			}
	...
	void Pythia::boostAndVertex( bool toLab, bool setVertex) {
		...
		if (setVertex && doVertexSpread) {
			...
			for (int i = 0; i < process.size(); ++i) {
				process[i].vProd( vertex);
			}
			for (int i = 0; i < event.size(); ++i) {
				event[i].vProd( vertex);
			}
		}
	...
	bool Pythia::doRHadronDecays( ) {
		...
		if ( !rHadrons.decay( event) ) {
			return false;
		}

		// Perform showers in resonance decay chains.
		if ( !partonLevel.resonanceShowers( process, event, false) ) {
			return false;
		}
	...
	void Pythia::stat() {
		...
		if (doProcessLevel) {
			if (showPrL) {
				processLevel.statistics(false);
			}
			if (reset) {
				processLevel.resetStatistics();
			}
		}

		// Statistics from other classes, currently multiparton interactions.
		if (showPaL) {
			partonLevel.statistics(false);
		}
		if (reset) {
			partonLevel.resetStatistics();
		}
	...
	int Pythia::readCommented(string line) {
		...
		int firstChar = line.find_first_not_of(" \n\t\v\b\r\f\a");
		if (int(line.size()) < firstChar + 2) {
			return 0;
		}

		// If first two nontrivial characters are /* or */ then done.
		if (line.substr(firstChar, 2) == "/*") {
			return +1;
		}
		if (line.substr(firstChar, 2) == "*/") {
			return -1;
	...
	bool Pythia::check(ostream& os) {
		...
		if (checkHistory) {
			...
			for (int i = 0; i < event.size(); ++i) {
				...
				for (int j = 0; j < int(mList.size()); ++j) {
					...
					if (!hasBeams && mList.size() == 1 && mList[0] == 0) {
						foundMatch = true;
					}
					if (!foundMatch) {
						bool oldPair = false;
						...
						if (!oldPair) {
							noMotDau.push_back( make_pair( mList[j], i) );
						}
					}
				}
				...
				for (int j = 0; j < int(dList.size()); ++j) {
					...
					if (!foundMatch) {
						...
						if (!oldPair) {
							noMotDau.push_back( make_pair( i, dList[j]) );
						}
					}
				}
			}
		...
		if (nErrEvent < nErrList) {
			...
			if (noMot.size() > 0) {
				...
				for (int i = 0; i < int(noMot.size()); ++i) {
					os << noMot[i] << " ";
				}
				os << "\n";
			}
			if (noDau.size() > 0) {
				...
				for (int i = 0; i < int(noDau.size()); ++i) {
					os << noDau[i] << " ";
				}
				os << "\n";
			}
	...
	PDF* Pythia::getPDFPtr(int idIn, int sequence, string beam) {
		...
		if (idIn == 990 && settings.mode("PDF:PomSet") == 2) {
			idIn = 111;
		}
		...
		if (abs(idIn) == 2212 || abs(idIn) == 2112) {
			...
			// Use internal sets.
				...
				tempPDFPtr = new GRV94L(idIn);
			}
			else if (pSetInt == 2) {
				tempPDFPtr = new CTEQ5L(idIn);
			}
			else if (pSetInt <= 6) {
		...
		// Lepton beam: neutrino, resolved charged lepton or unresolved ditto.
			...
			if (abs(idIn) % 2 == 0) {
				tempPDFPtr = new NeutrinoPoint(idIn);
			}
